...
carnegie = read.csv('artifacts/carnegie-r1.csv')
...
herd_post = herd |> filter(herd_year >= 2010) # 20090
herd_pre  = herd |> filter(herd_year < 2010) # n = 41956


############ JOIN DATA BY FUZZY MATCHING, POST 2010
...
# relocate
herd_post = herd_post |> relocate(carnegie_city, .before = herd_state)
herd_post = herd_post |> relocate(carnegie_name_clean, .before = carnegie_city)
herd_post = herd_post |> relocate(stabbr, .after = herd_state)
herd_post = herd_post |> relocate(herd_carnegieCycle, .before = funding_type)


############ PRE DATA WRANGLING
# institution names vary in HERD data (e.g., state versus saint, u versus univ etc)
# will eliminate true pos in fuzzy match (ex ca v california)
...
# filter out all but top 50
top50_names <- herd_pre |>
...
# join to new data to filter first section
herd_pre_early <- herd_pre |>
  filter(herd_year <= 1994) |>
  semi_join(top50_names, by = c("herd_name_clean", "herd_year"))

# filter pre for the data that was parsed in previous
...
#check rows
nrow(herd_pre) # n = 21515
table(herd_pre$herd_year)

# look for unique names
herd_distinct = distinct(herd_pre, herd_name_clean) # n = 1326
carnegie_distinct = carnegie |> distinct(unitid, carnegie_name_clean) # n = 177
...
herd_pre |>
  ...
  summarise(n_inst = n_distinct(herd_name_clean),
            n_rows = n())
# flag               n_inst n_rows
# 1 Good Match            105   2578
# 2 Possible Bad Match     120    2578


## INSPECT, artifacts/fuzzy match-manual inspection.xlsx
# multiple values per carnegie cycle  per institution b/c HERD is annaul
# filter by distinct to get inst/year pairs to manually inspect
inspect = herd_pre |>
  select(flag, dist, herd_name_clean, carnegie_name_clean, carnegie_city, herd_carnegieCycle, carnegie_year, dist) |>
  distinct()

# commented out b/c final inspection complete
# write.csv(inspect, "artifacts/fuzzy-match-inspect.csv",  row.names = FALSE)
...
# join
herd = rbind(herd_pre, herd_post)

# clean up vars for analysis
herd = herd |> select(herd_name_clean, carnegie_city, herd_state, herd_year, herd_carnegieCycle, funding_type, data)
...
# save
write.csv(herd, "data/herd.csv", row.names = FALSE)
